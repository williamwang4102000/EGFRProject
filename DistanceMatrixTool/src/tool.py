...
class TutorialTool(HtmlToolInstance):
    ...
    def __init__(self, session, tool_name):
        ...
        self.display_name = "Distance Matrix Tool"
    ...
    def handle_scheme(self, url):
        ...
        if command == "update_models":
            self.update_models()
            return
        elif command in ["dist"]:
            # Collect the optional parameters from URL query parameters
            # and construct a command to execute
            from urllib.parse import parse_qs
            query = parse_qs(url.query())
            self.session.logger.info('query:' + str(query))
            import sys
            for p in sys.path:
                self.session.logger.info(str(p))

            from chimerax.core.commands import run
        elif command in ["pdbdist"]:
            from .Dist_Calc import pdb_dist_matrix
            pdb_dist_matrix('1ATP','1ATP.pdb')
            self.session.logger.info('test')
        else:
    ...
    def update_models(self, trigger=None, trigger_data=None):
        ...
        for m in self.session.models:
            if("<class 'chimerax.atomic.structure.AtomicStructure'>" == str(type(m))):
                self.session.logger.info(str(type(m)))
                for c in m.chains:
                    options.append((c, c.chain_id))
